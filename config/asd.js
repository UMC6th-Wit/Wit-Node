...
fs.readFile('products.json', 'utf8', async (err, data) => {
  ...
  // INSERT 문 생성
  let insertQuery =
    'INSERT INTO product (id, name, description, won_price, en_price, image, sales_area, product_type) VALUES ';

  const values = products.map((product, index) => {
    return `(${index + 1}, '${product.name}', '${
      product.description
    }', ${parseInt(product.won_price.replace(/[^0-9]/g, ''))}, ${parseInt(
      product.en_price.replace(/[^0-9]/g, '')
    )}, '${product.image}', '${product.sales_area}', '${
